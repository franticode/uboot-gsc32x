...
struct gsc32_clk_priv {
	void __iomem *iobase;
	void __iomem *syscfg_base;
};

static ulong gsc32_get_pll_rate(struct gsc32_clk_priv *priv)
{
...
static ulong gsc32_get_sysclk(struct gsc32_clk_priv *priv)
{
	ulong v;
	ulong hz;
	ulong div, frcdiv;
...
static ulong gsc32_get_pbclk(struct gsc32_clk_priv *priv, int periph)
{
	void __iomem *reg;
	ulong div, clk_freq;
	...
	clk_freq = gsc32_get_sysclk(priv);
...
static ulong gsc32_get_cpuclk(struct gsc32_clk_priv *priv)
{
	return gsc32_get_pbclk(priv, PB7CLK);
}

static ulong gsc32_set_refclk(struct gsc32_clk_priv *priv, int periph,
...
static ulong gsc32_get_refclk(struct gsc32_clk_priv *priv, int periph)
{
	...
	switch (rosel) {
	case ROCLK_SRC_SCLK:
		parent_rate = gsc32_get_cpuclk(priv);
		break;
	case ROCLK_SRC_SPLL:
		parent_rate = gsc32_get_pll_rate(priv);
		break;
	default:
		parent_rate = 0;
		break;
...
static ulong gsc32_get_mpll_rate(struct gsc32_clk_priv *priv)
{
	u32 v, idiv, mul;
	u32 odiv1, odiv2;
	u64 rate;
...
static int gsc32_mpll_init(struct gsc32_clk_priv *priv)
{
	u32 v, mask;
...
static void gsc32_clk_init(struct udevice *dev)
{
	const void *blob = gd->fdt_blob;
	struct gsc32_clk_priv *priv;
	ulong rate, pll_hz;
	char propname[50];
	int i;

	priv = dev_get_priv(dev);
	pll_hz = gsc32_get_pll_rate(priv);

	/* Initialize REFOs as not initialized and enabled on reset. */
	for (i = REF1CLK; i <= REF5CLK; i++) {
		snprintf(propname, sizeof(propname),
			 "godson,refo%d-frequency", i - REF1CLK + 1);
		rate = fdtdec_get_int(blob, dev->of_offset, propname, 0);
		if (rate)
			gsc32_set_refclk(priv, i, pll_hz, rate, ROCLK_SRC_SPLL);
	}

	/* Memory PLL */
	gsc32_mpll_init(priv);
}

static ulong gsc32_get_rate(struct clk *clk)
{
	struct gsc32_clk_priv *priv = dev_get_priv(clk->dev);
	ulong rate;

	switch (clk->id) {
	case PB1CLK ... PB7CLK:
		rate = gsc32_get_pbclk(priv, clk->id);
		break;
	case REF1CLK ... REF5CLK:
		rate = gsc32_get_refclk(priv, clk->id);
		break;
	case PLLCLK:
		rate = gsc32_get_pll_rate(priv);
		break;
	case MPLL:
		rate = gsc32_get_mpll_rate(priv);
		break;
	default:
		rate = 0;
		break;
...
static ulong gsc32_set_rate(struct clk *clk, ulong rate)
{
	struct gsc32_clk_priv *priv = dev_get_priv(clk->dev);
	ulong pll_hz;
	...
	case REF1CLK ... REF5CLK:
		pll_hz = gsc32_get_pll_rate(priv);
		gsc32_set_refclk(priv, clk->id, pll_hz, rate, ROCLK_SRC_SPLL);
		break;
	default:
		break;
	}
...
static struct clk_ops gsc32_gsc32_clk_ops = {
	.set_rate = gsc32_set_rate,
	.get_rate = gsc32_get_rate,
};

static int gsc32_clk_probe(struct udevice *dev)
{
	struct gsc32_clk_priv *priv = dev_get_priv(dev);
	fdt_addr_t addr;
	fdt_size_t size;
	...
	priv->syscfg_base = gsc32_get_syscfg_base();
...
static const struct udevice_id gsc32_clk_ids[] = {
	{ .compatible = "godson,gsc32-clk"},
	{}
};

U_BOOT_DRIVER(gsc32_clk) = {
	.name		= "gsc32_clk",
	.id		= UCLASS_CLK,
	.of_match	= gsc32_clk_ids,
	.flags		= DM_FLAG_PRE_RELOC,
	.ops		= &gsc32_gsc32_clk_ops,
	.probe		= gsc32_clk_probe,
	.priv_auto_alloc_size = sizeof(struct gsc32_clk_priv),
};
