...
struct gsc32_reg_nvm {
	struct gsc32_reg_atomic ctrl;
	struct gsc32_reg_atomic key;
	struct gsc32_reg_atomic addr;
	struct gsc32_reg_atomic data;
};
...
static struct gsc32_reg_nvm *nvm_regs_p;
...
static void gsc32_flash_bank_init(flash_info_t *info,
...
static int gsc32_flash_probe(struct udevice *dev)
{
	...
	for (idx = 0, info = &flash_info[0]; list < end;) {
		...
		if (!strncmp(list, "nvm", len)) {
			...
			gsc32_flash_bank_init(info, CKSEG0ADDR(addr), size);
			info++;
		}
		idx += addrc + sizec;
		list += len + 1;
...
static const struct udevice_id gsc32_flash_ids[] = {
	{ .compatible = "godson,gsc32-flash" },
	{}
};

U_BOOT_DRIVER(gsc32_flash) = {
	.name	= "gsc32_flash",
	.id	= UCLASS_MTD,
	.of_match = gsc32_flash_ids,
	.probe	= gsc32_flash_probe,
};
